...
const App: React.FC = () => {
  ...
  const [knowledgeBase, setKnowledgeBase] = useState<KnowledgeBase>(() => {
    ...
    return saved ? JSON.parse(saved) : { 
        ...
        globalStrategyRules: {
            ...
            seamlessTransitionLogic: true,
            viralFormulaId: 'f_01',
            algorithmOptimizationLevel: 85
        }
    };
  });

  // Lưu KnowledgeBase vào LocalStorage khi thay đổi
  useEffect(() => {
  ...
  const runAutonomousCycle = async () => {
      ...
      try {
          ...
          addAutoPilotLog("PLAN", `Thiết kế kịch bản với Logic Seamless cho ${target.product_name}...`);
          ...
          addAutoPilotLog("DONE", `Vận hành hoàn tất: ${bestTime}`, "success");
        ...
        <header className="h-16 border-b border-slate-800 flex items-center justify-between px-6 bg-slate-950/50 backdrop-blur-md sticky top-0 z-30">
            <div className="flex items-center gap-6">
                <div className="flex items-center gap-2">
                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">NEURAL STATUS:</span>
                    ...
                    {autoPilotActive && (
                        <div className="flex items-center gap-1 ml-2 text-green-500">
                            ...
                            <span className="text-[8px] font-black uppercase">Real-time Strategy Synced</span>
                        </div>
                    )}
                </div>
            </div>
            ...
            <div className="flex items-center gap-4">
                <div className="bg-slate-900/50 border border-slate-800 rounded-xl px-3 py-1.5 flex items-center gap-2">
                   ...
                   <span className="text-[10px] font-black text-white uppercase tracking-tighter">Enterprise Protocol</span>
                </div>
            </div>
        </header>
