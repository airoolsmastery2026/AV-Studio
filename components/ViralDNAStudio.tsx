      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          <div className="lg:col-span-8 space-y-6">
                  ...
                  <div className="bg-[#070B14]/80 border border-slate-800 rounded-[32px] p-6 md:p-10 animate-fade-in space-y-8 relative overflow-hidden backdrop-blur-xl shadow-2xl">
                      ...
                      <div className="space-y-4">
                          <div className="flex justify-between items-center px-1">
                            <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">Parallel Multimodal Recon ({channels.length})</span>
                            <div className="flex gap-4">
                              <button onClick={clearChannels} className="text-[9px] font-black text-slate-600 hover:text-red-500 uppercase flex items-center gap-1.5 transition-colors"><Eraser size={14} /> Reset Radar</button>
                              <button onClick={addChannel} className="text-[9px] font-black text-primary hover:text-white uppercase flex items-center gap-1.5 transition-colors"><Plus size={14} /> Add Target</button>
                            </div>
                          </div>
                          ...
                          <div className="max-h-[320px] overflow-y-auto pr-2 custom-scrollbar space-y-3">
                            {channels.map((channel, i) => (
                                <div key={channel.id} className={`group relative flex items-center gap-3 bg-slate-950/60 p-3.5 rounded-2xl border transition-all shadow-lg focus-within:border-primary/50 ${channel.status === 'analyzing' ? 'border-primary/40 bg-primary/5 animate-pulse' : 'border-slate-800/60 hover:border-primary/30'}`}>
                                    ...
                                    <input value={channel.url} onChange={(e) => updateChannelUrl(channel.id, e.target.value)} disabled={channel.status === 'analyzing'} placeholder="Dán URL video đối thủ (TikTok/YT Shorts)..." className="bg-transparent border-none outline-none text-white font-mono text-xs flex-1 placeholder:text-slate-800 disabled:opacity-50" />
                                </div>
                            ))}
                          </div>
                      </div>
                      ...
                      <div className="pt-6 flex justify-center">
                          <NeonButton onClick={handleRunAnalysis} disabled={status === 'analyzing' || channels.every(c => !c.url.trim())} size="lg" className="min-w-[360px] h-16 shadow-neon">
                              {status === 'analyzing' ? <Loader2 className="animate-spin" /> : <Radar size={20} />}
                              {status === 'analyzing' ? `Analyzing targets...` : `Initiate Multimodal Recon`}
                          </NeonButton>
                      </div>
                  </div>
              )}
                  ...
                  <div className="animate-fade-in space-y-6">
                      {!generatedPlan ? (
                          <div className="bg-[#070B14]/80 border border-slate-800 rounded-[32px] p-8 md:p-12 flex flex-col items-center justify-center text-center space-y-8 backdrop-blur-xl shadow-2xl">
                               <div className="w-20 h-20 bg-slate-950 rounded-3xl flex items-center justify-center border border-slate-800/50 opacity-40 shadow-inner group relative">
                                   <div className="absolute inset-0 bg-primary/5 rounded-3xl animate-pulse"></div>
                                   <BrainCircuit size={48} className="text-primary" />
                               </div>
                               <div className="space-y-2">
                                   <h4 className="text-xl font-black text-white uppercase tracking-tighter">Advanced Production Locked</h4>
                               ...
                               <input value={studioSettings.topic} onChange={(e) => setStudioSettings({...studioSettings, topic: e.target.value})} placeholder="What's the core mission or product?" className="w-full max-w-md bg-slate-950 border border-slate-800 rounded-2xl p-4 text-xs md:text-sm text-white font-bold outline-none focus:border-primary shadow-inner transition-all" />
              ...
              {activeStudioTab === 'library' && (
                <div className="animate-fade-in space-y-6">
                    <div className="bg-[#070B14]/80 border border-slate-800 rounded-[32px] p-6 md:p-10 backdrop-blur-xl shadow-2xl space-y-8">
                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                            <h3 className="text-2xl font-black text-white uppercase tracking-tighter flex items-center gap-3"><Library className="text-primary" size={24} /> Video Archive</h3>
                            <div className="w-full md:w-80 relative group">
                                <div className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-600 group-focus-within:text-primary transition-colors"><SearchIcon size={18} /></div>
                                <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search archive..." className="w-full bg-slate-950 border border-slate-800 rounded-2xl py-3.5 pl-12 pr-4 text-xs text-white font-bold outline-none focus:border-primary shadow-inner transition-all" />
                            </div>
                        </div>
                                ...
                                {filteredArchive.map((video) => (
                                    <div key={video.id} className="bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden group hover:border-primary/40 transition-all shadow-xl flex flex-col">
                                        <div className="aspect-[9/16] bg-black relative flex items-center justify-center overflow-hidden">
                                            {video.thumbnail ? <img src={video.thumbnail} className="w-full h-full object-cover opacity-60 group-hover:scale-105 group-hover:opacity-100 transition-all duration-500" alt={video.title} /> : <div className="p-10 bg-slate-950/50 rounded-full border border-white/5"><VideoIcon size={32} className="text-slate-700" /></div>}
                                            <a href={video.url} target="_blank" className="absolute top-4 right-4 p-2 bg-black/60 rounded-lg text-white opacity-0 group-hover:opacity-100 transition-opacity"><Download size={16}/></a>
                                        </div>
                                        <div className="p-5 space-y-4 flex-1 flex flex-col justify-between">
                                            <div className="space-y-1">
                                                ...
                                                <div className="flex items-center gap-2 text-slate-500"><Clock size={10} /><span className="text-[9px] font-mono">{new Date(video.timestamp).toLocaleDateString()}</span></div>
                                            </div>
                                        </div>
                                    </div>
                                ))}
