...
const AutoPilotDashboard: React.FC<AutoPilotDashboardProps> = ({ 
    ...
    currentMission, onNavigateToAnalytics, t
}) => {
    const logsEndRef = useRef<HTMLDivElement>(null);
    useEffect(() => { logsEndRef.current?.scrollIntoView({ behavior: 'smooth' }); }, [logs]);

    const MISSION_STAGES = [
        { id: AppStatus.HUNTING, label: t.ap_stage_a, desc: 'Google Search Digital Hunt' },
        { id: AppStatus.PLANNING, label: t.ap_stage_b, desc: 'VidIQ SEO Blueprinting' },
        { id: AppStatus.RENDERING, label: t.ap_stage_c, desc: 'Neural Asset Engine' },
        { id: AppStatus.SCHEDULING, label: t.ap_stage_d, desc: 'Smart Platform Push' }
    ];
        ...
        <div className="animate-fade-in space-y-6 pb-20">
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl flex flex-col xl:flex-row justify-between items-center gap-8 relative overflow-hidden">
                ...
                <div className="flex items-center gap-6">
                    ...
                    <div>
                        <h2 className="text-3xl font-black text-white tracking-tighter uppercase">{t.ap_title} <span className="text-[10px] bg-primary px-2 py-1 rounded ml-2">AUTO-PILOT AI</span></h2>
                ...
                <div className="flex gap-10 px-10 py-4 bg-black/40 rounded-2xl border border-white/5 shadow-inner">
                    <div className="text-center">
                        <div className="text-[10px] text-slate-500 font-black uppercase mb-1">{t.ap_missions}</div>
                        <div className="font-mono text-xl text-primary font-black">{stats.cyclesRun}</div>
                    </div>
                    <div className="text-center">
                        <div className="text-[10px] text-slate-500 font-black uppercase mb-1">{t.ap_engaged}</div>
                        <div className="font-mono text-xl text-white font-black">{stats.videosCreated}</div>
                    </div>
                    <div className="text-center">
                        <div className="text-[10px] text-slate-500 font-black uppercase mb-1">{t.ap_uptime}</div>
                ...
                <div className="flex gap-3">
                   <NeonButton onClick={() => setIsRunning(!isRunning)} variant={isRunning ? 'danger' : 'primary'} size="lg" className="min-w-[220px] h-16">
                       ...
                       {isRunning ? t.ap_halt : t.ap_engage}
                   </NeonButton>
                </div>
            </div>
                ...
                <div className="lg:col-span-3 space-y-6">
                    <div className="bg-slate-950 border border-slate-800 rounded-[40px] p-8 shadow-2xl relative overflow-hidden">
                        <h3 className="text-xs font-black text-white uppercase tracking-widest mb-8 flex items-center gap-3">
                           <Activity size={20} className="text-primary" /> {t.ap_pipeline}
                        </h3>
                    ...
                    <div className="bg-slate-900 border border-slate-800 rounded-[40px] p-8 shadow-xl">
                        <h3 className="text-sm font-black text-white uppercase tracking-widest mb-6 flex items-center gap-2"><Target size={16} className="text-primary"/> {t.ap_target_sector}</h3>
                        <div className="space-y-4">
                            ...
                            >
                                ...
                                <option value="AI_SAAS_TOOLS">🤖 {t.niche_ai_saas}</option>
                                <option value="ML_PLATFORMS">🧠 {t.niche_ml}</option>
                                <option value="AI_PRODUCTIVITY_HACKS">⚡ {t.niche_productivity}</option>
                                <option value="PASSIVE_INCOME">💰 {t.niche_passive_income}</option>
                                <option value="SMART_HOME_GADGETS">🏠 {t.niche_smart_home}</option>
                            </select>
                    ...
                    <div className="bg-slate-950 border border-slate-800 rounded-[40px] p-8 shadow-2xl relative overflow-hidden flex flex-col min-h-[600px]">
                        <div className="flex justify-between items-center mb-8 border-b border-slate-800 pb-4">
                            <h3 className="text-sm font-black text-white uppercase tracking-widest flex items-center gap-3">
                                <SearchCode size={20} className="text-primary" /> {t.ap_intel_report}
                            </h3>
                            {currentMission && (
                                <div className="flex items-center gap-2 px-3 py-1 bg-primary/10 border border-primary/20 rounded-full">
                                    <Flame size={12} className="text-primary" />
                                    <span className="text-[10px] font-black text-primary uppercase">{t.potential_label}: {currentMission.trending_score || 85}%</span>
                                </div>
                            )}
                        </div>
                        ...
                        {currentMission ? (
                                ...
                                {currentMission.vidiq_score && (
                                    <div className="grid grid-cols-3 gap-4 bg-slate-900/50 p-6 rounded-3xl border border-primary/20 shadow-xl relative overflow-hidden">
                                        ...
                                        <div className="flex flex-col items-center justify-center text-center border-r border-slate-800">
                                            ...
                                            <div className="text-[9px] text-slate-500 font-black uppercase mt-1">{t.seo_score}</div>
                                        </div>
                                        <div className="flex flex-col items-center justify-center text-center border-r border-slate-800">
                                            ...
                                            <div className="text-[9px] text-slate-500 font-black uppercase mt-1">{t.keyword_difficulty}</div>
                                        </div>
                                        <div className="flex flex-col items-center justify-center text-center">
                                            <div className="text-lg font-black text-accent">{currentMission.vidiq_score.trending_momentum}%</div>
                                            <div className="text-[9px] text-slate-500 font-black uppercase mt-1">{t.viral_momentum}</div>
                                        </div>
                                    </div>
                                )}
                                ...
                                {currentMission.vidiq_score?.suggested_tags && (
                                    <div className="space-y-3">
                                        <div className="text-[10px] text-slate-500 font-black uppercase tracking-widest flex items-center gap-2">
                                            <Sparkles size={14} className="text-primary" /> {t.suggested_tags}
                                        </div>
                            ...
                            <div className="flex-1 flex flex-col items-center justify-center text-center opacity-30 space-y-6">
                                ...
                                <div className="space-y-2">
                                    <h4 className="text-xl font-black text-white uppercase tracking-tighter">{t.ap_hunting_msg}</h4>
                ...
                <div className="lg:col-span-3 space-y-6">
                    <div className="bg-black border border-slate-800 rounded-[40px] overflow-hidden flex flex-col h-[600px] shadow-2xl relative">
                        <div className="bg-slate-900 px-8 py-5 border-b border-slate-800 flex justify-between items-center z-10">
                            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-2">
                                <Cpu size={14} /> {t.ap_log_title}
                            </span>
                            <div className="flex gap-2">
                                <span className="px-2 py-0.5 bg-green-500/10 text-green-500 text-[8px] font-black rounded uppercase border border-green-500/20">{t.status_online}</span>
                            </div>
                        </div>
                    ...
                    <div className="bg-gradient-to-br from-primary/10 to-transparent border border-primary/20 rounded-[40px] p-8">
                        <div className="flex items-center gap-3 mb-4">
                            ...
                            <span className="text-[10px] font-black text-white uppercase tracking-widest">{t.ap_protocol_active}</span>
                        </div>
