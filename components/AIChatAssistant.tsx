...
const QUICK_ACTIONS = [
    { label: 'Market Recon', icon: Search, text: 'Quét thị trường tìm trend ngay.' },
    { label: 'Auto-Pilot On', icon: Zap, text: 'Kích hoạt robot sản xuất AI.' },
    { label: 'Go to Lab', icon: Navigation, text: 'Mở Viral Studio.' }
];
...
const AIChatAssistant: React.FC<AIChatAssistantProps> = ({ apiKey, appContext, onCommand }) => {
  ...
  // Khởi tạo session chat từ local storage
  useEffect(() => {
    ...
    try {
      ...
      const sessionPromise = ai.live.connect({
        ...
        config: {
          ...
          systemInstruction: "Bạn là AI Commander. Hãy đàm thoại một cách chuyên nghiệp và sắc sảo. Hỗ trợ người dùng vận hành AV Studio."
        },
        callbacks: {
          onopen: () => {
  ...
  const stages = ["Standby", "Retrieving Context", "Neural Thinking", "Optimizing Dialogue"];
    ...
    <>
      ...
      <div className={`fixed bottom-6 right-6 z-[100] transition-all duration-700 ${isOpen ? 'scale-0 translate-y-20' : 'scale-100 translate-y-0'}`}>
        <button 
            ...
            className="w-16 h-16 rounded-[24px] bg-slate-900 border border-primary/40 flex items-center justify-center shadow-neon hover:shadow-neon-hover group overflow-hidden active:scale-90 transition-all"
        >
          ...
          <div className="p-6 bg-slate-950/90 backdrop-blur-2xl border-b border-slate-800 flex justify-between items-center shrink-0">
            <div className="flex items-center gap-4">
              <div className="relative">
                 ...
                 {isLiveMode && <div className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full animate-ping"></div>}
              </div>
              <div>
                ...
                <span className="text-[10px] text-primary/60 font-black uppercase tracking-[0.2em] flex items-center gap-2">
                    <div className="w-1.5 h-1.5 bg-primary/60 rounded-full"></div> 
                    {isLiveMode ? 'Live Dialogue Active' : 'Strategy Intelligence'}
                </span>
              </div>
            </div>
            <div className="flex gap-2">
              <button 
                ...
                className={`p-2.5 rounded-xl transition-all ${isAudioEnabled ? 'bg-primary/10 text-primary border border-primary/20' : 'text-slate-500 hover:bg-slate-800'}`}
              ...
              <button 
                ...
                className={`p-2.5 rounded-xl transition-all ${isLiveMode ? 'bg-red-500 text-white shadow-neon' : 'text-slate-500 hover:bg-slate-800'}`}
                title="Chế độ đàm thoại"
              >
                {isLiveMode ? <Mic size={20} /> : <MicOff size={20} />}
              </button>
             ...
             {isLiveMode ? (
                <div className="h-full flex flex-col items-center justify-center text-center space-y-12 animate-fade-in">
                    <div className="relative">
                        <div className="w-40 h-40 bg-primary/5 rounded-full flex items-center justify-center border border-primary/20 animate-[pulse_1.5s_infinite]">
                            <div className="flex items-end gap-1.5 h-14">
                                ...
                                <div className="w-2 bg-primary rounded-full animate-[wave_1s_infinite_0.4s]" style={{ height: '65%' }}></div>
                    ...
                    <div className="space-y-4">
                        <h4 className="font-black text-white text-2xl uppercase tracking-tighter">VOICE COMMAND LINK</h4>
                        <p className="text-[11px] text-slate-500 uppercase font-black tracking-[0.4em] animate-pulse">Commander is listening...</p>
                    </div>
                    <button 
                        onClick={toggleLiveMode} 
                        className="px-8 py-3 bg-red-500/10 border border-red-500/40 text-red-500 rounded-full text-[10px] font-black uppercase tracking-widest hover:bg-red-500 hover:text-white transition-all"
                    >
                        End Conversation
                    </button>
                </div>
                      ...
                      <div key={i} className={`flex flex-col ${m.role === 'user' ? 'items-end' : 'items-start'} animate-fade-in group`}>
                          <div className={`relative max-w-[85%] p-5 rounded-[26px] text-sm leading-relaxed shadow-2xl border transition-all duration-300 ${
                            m.role === 'user' 
                              ...
                              {m.role === 'model' && (
                                <button 
                                    onClick={() => playTTS(m.text, m.id, m.detected_lang, m.sentiment)}
                                    className={`absolute -right-12 top-1/2 -translate-y-1/2 p-2.5 rounded-2xl bg-slate-900 border border-slate-800 hover:border-primary transition-all opacity-0 group-hover:opacity-100 ${isSpeakingId === m.id ? 'opacity-100 border-primary text-primary animate-pulse' : 'text-slate-500 shadow-xl'}`}
                                >
                              ...
                              {/* Sentiment Feedback */}
                              {m.role === 'model' && m.sentiment && m.sentiment !== 'neutral' && (
                                 <div className={`absolute -top-3 -left-3 p-2 rounded-full border shadow-2xl scale-110 ${
                                    m.sentiment === 'happy' ? 'bg-green-500 border-green-400 text-white' :
                                    m.sentiment === 'urgent' ? 'bg-red-600 border-red-500 text-white' :
                                    'bg-indigo-600 border-indigo-500 text-white'
                                 }`}>
                                    {m.sentiment === 'happy' ? <Smile size={12} /> : m.sentiment === 'urgent' ? <AlertOctagon size={12} /> : <Brain size={12} />}
                                 </div>
                              )}
                          </div>
                            ...
                            <div className="flex flex-wrap gap-2.5 mt-5 animate-fade-in pl-2">
                               {m.suggestions.map((s, idx) => (
                                 <button 
                                    ...
                                    className="px-4 py-2 bg-slate-950 border border-slate-800 rounded-full text-[10px] font-black text-slate-400 hover:text-white hover:border-primary hover:bg-primary/10 transition-all flex items-center gap-2 group/chip uppercase tracking-tight"
                                 >
                                   <Zap size={10} className="text-primary/70 group-hover/chip:text-primary" /> {s}
                                 </button>
                               ))}
                            </div>
                          )}
                      ...
                      <div className="flex justify-start animate-fade-in">
                          <div className="bg-slate-900/50 backdrop-blur-sm border border-slate-800 p-6 rounded-[28px] rounded-tl-none flex flex-col gap-4 min-w-[280px] shadow-2xl relative overflow-hidden">
                             ...
                             <div className="flex items-center gap-4">
                                ...
                                <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Processing Signal</span>
                             </div>
                ...
                <div className="flex gap-2.5 overflow-x-auto pb-5 custom-scrollbar no-scrollbar">
                    {QUICK_ACTIONS.map((action, idx) => (
                        <button 
                            ...
                            className="flex items-center gap-2.5 px-5 py-3 bg-slate-900 border border-slate-800 rounded-2xl text-[10px] font-black text-slate-400 hover:text-white hover:border-primary transition-all whitespace-nowrap shrink-0 uppercase tracking-tighter group/action"
                        >
                            <action.icon size={12} className="text-primary opacity-60 group-hover/action:opacity-100 group-hover/action:scale-125 transition-all" />
                            {action.label}
                        </button>
                    ))}
                </div>
                <div className="flex gap-4">
                    <div className="relative flex-1 group">
                        <input 
                            ...
                            placeholder="Type a creative command..."
                            className="w-full bg-slate-900 border border-slate-800 rounded-[22px] px-6 py-4.5 text-sm text-white focus:outline-none focus:border-primary/50 transition-all placeholder:text-slate-800 shadow-inner"
                        />
                        <div className="absolute right-5 top-1/2 -translate-y-1/2 text-slate-800 pointer-events-none uppercase text-[8px] font-black tracking-[0.3em] hidden md:block">
                           Neural In
                        </div>
                    </div>
                    <button 
                        ...
                        className="w-14 h-14 bg-primary rounded-[22px] text-white flex items-center justify-center hover:bg-primary/80 transition-all disabled:opacity-20 shadow-neon active:scale-95 shrink-0"
                    >
                        <Send size={24} />
                    </button>
                </div>
